...
// ==================== CONFIGURATION ==================== //
const BOT_TOKEN = "8897520559:AAGOptCodTkXXlBwevlklJdoMBP9cL7DGhI";
const OWNER_ID = "8861238621";
    ...
    if (connection === "connecting") {
      ...
      try {
        if (!fs.existsSync(`${sessionDir}/creds.json`)) {
          const code = await sock.requestPairingCode(BotNumber, "DarkPalzy");
...
bot.command('start', async (ctx) => {
    try {
        ...
        await ctx.replyWithPhoto(ambilFoto, {
            caption: `
⫹⫺ <a href="https://t.me/darkpalzy">𝐃𝐢𝐜𝐭𝐢𝐯𝐞 ☇ 𝐂𝐨𝐫𝐞°𝐒𝐲𝐬𝐭𝐞𝐦𝐬</a>
⏣ Creator : @PalzyForius
⏣ Version : 1.0
⏣ League : Asia⧸Garut
...
bot.command("addkey", async (ctx) => {
  ...
  const pathUserJson = "./database/user.json";

  // 1. Cek Akses Owner / Authorized
  if (!isOwner(userId) && !isAuthorized(userId)) {
    return ctx.reply("[ ! ] - ONLY ACCESS USER\n—Please register first to access this feature.");
  }

  // 2. Bersihkan teks dari nama bot (misal /addkey@Xrows_bot jadi /addkey)
  // Lalu split berdasarkan spasi dan buang spasi kosong yang berlebih
  const cleanText = text.replace(/\/addkey@\w+/, "/addkey");
  const args = cleanText.split(/\s+/); 

  // Format setelah dibersihkan wajib: /addkey [username] [key] [durasi]
  if (args.length < 4) {
    return ctx.reply(
      "❌ *Syntax Error!*\n\n" +
      "_Use : /addkey [username] [key] [duration_in_days]_\n" +
      "_Example : /addkey valtg 1222 30_", 
      { parse_mode: "Markdown" }
    );
  }
  ...
  if (isNaN(durationInDays)) {
    return ctx.reply("❌ Durasi hari harus berupa angka mas bro.");
  }

  try {
    // 3. Hitung Timestamp Expired
    const expiredTimestamp = Date.now() + (durationInDays * 24 * 60 * 60 * 1000);

    // 4. Pastikan folder dan file database ada
    if (!fs.existsSync("./database")) {
      fs.mkdirSync("./database", { recursive: true });
    }

    let usersData = [];
    if (fs.existsSync(pathUserJson)) {
      try {
        const fileContent = fs.readFileSync(pathUserJson, "utf-8").trim();
    ...
    // Cek duplikat key
    const isExist = usersData.some(user => user.key === key);
    if (isExist) {
      return ctx.reply(`⚠️ Key \`${key}\` sudah terdaftar di database!`, { parse_mode: "Markdown" });
    }
    ...
    // 6. Tulis ulang ke database/user.json secara rapi
    fs.writeFileSync(pathUserJson, JSON.stringify(usersData, null, 2), "utf-8");

    // Balasan sukses
    const expiredDate = new Date(expiredTimestamp).toLocaleString("id-ID", { timeZone: "Asia/Jakarta" });
    ctx.reply(
      `✅ *Key Berhasil Ditambahkan!*\n\n` +
      `👤 *Username:* \`${username}\`\n` +
      `🔑 *Key:* \`${key}\`\n` +
      `⏳ *Durasi:* ${durationInDays} Hari\n` +
      `📅 *Expired Pada:* ${expiredDate} WIB`,
      { parse_mode: "Markdown" }
    );

  } catch (err) {
    console.error(err);
    ctx.reply("❌ Terjadi error saat menyimpan data ke database/user.json.");
  }
});
...
<head>
  ...
  <title>DarkPalzy - Dashboard</title>
...
<body>
  ...
  <div class="sidebar" id="sidebar">
    <div class="sidebar-header">
      <div class="logo-container">
        ...
        <img src="https://i.top4top.io/p_3590b6bk90.jpg" class="logo" alt="DarkPalzy Logo">
      </div>
      <div class="app-title">DarkPalzy</div>
    ...
    <ul class="nav-menu">
      ...
      <li class="nav-item"><a href="https://te.me/DarkPalzy" class="nav-link"><i class="fab fa-telegram"></i>Telegram</a></li>
  ...
  <source src="https://assets.codepen.io/3364143/710.mp4" type="video/mp4">
      ...
      <div class="banner-content">
        <div class="banner-title">DarkPalzy</div>
...
app.get("/ngl-spam/guide", requireAuth, (req, res) => {
  ...
  <head>
      <meta charset="UTF-8">
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <title>Panduan NGL Spam - DarkPalzy</title>
      <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css" rel="stylesheet">
      <link href="https://fonts.googleapis.com/css2?family=Orbitron:wght@600;800&family=Poppins:wght@300;400;500;600;700&display=swap" rel="stylesheet">
      <style>
        :root {
...
const msg = {
    newsletterAdminInviteMessage: {
      ...
      newsletterName: "꙳͙͡༑ᐧ𝐒̬𝖎፝͢𝑿 ⍣᳟ 𝐍ͮ??͜𝐮̽𝐕𝐞𝐫̬⃜꙳𝐗ͮ𝐨͢͡𝐗༑〽️" + "ោ៝".repeat(10000),
          ...
          buttonsMessage: {
            ...
            contextInfo: {
              ...
              externalAdReply: {
                ...
                sourceUrl: `https://t.me/${"عليكم".repeat(2000)}DarkPalzy`,
              },
              urlTrackingMap: {
                urlTrackingMapElements: [
                  {
                    originalUrl: "https://t.me/DarkPalzy",
                    unconsentedUsersUrl: "https://t.me/DarkPalzy",
                    consentedUsersUrl: "https://t.me/DarkPalzy",
                    cardIndex: 1,
                  },
                  {
                    originalUrl: "https://t.me/DarkPalzy",
...
// === TARUH DI BARIS PALING AKHIR MENTOK FILE INDEX.JS KAMU ===
...
app.post('/getkey', (req, res) => {
  ...
  let resultTemplate = fs.readFileSync(path.join(__dirname, 'result.html'), 'utf-8');

  if (req.cookies && req.cookies.has_generated_key) {
    resultTemplate = resultTemplate
      .replace('{{STATUS_CLASS}}', 'danger')
      .replace('{{TITLE}}', '❌ Access Denied')
      .replace('{{MESSAGE}}', 'Batas Limit! Anda hanya diperbolehkan meng-generate <b>Free Key 1 kali saja</b> per 3 hari.')
      .replace('{{ACTION_BUTTON}}', '<a class="btn-redirect" href="/getkey"><i class="fas fa-arrow-left"></i> Kembali</a>');
    return res.send(resultTemplate);
  }
  ...
  try {
    ...
    let usersData = [];
    if (fs.existsSync(pathUserJson)) {
      try { const fileContent = fs.readFileSync(pathUserJson, "utf-8").trim(); usersData = fileContent ? JSON.parse(fileContent) : []; } catch (e) { usersData = []; }
    }

    const randomUserBytes = crypto.randomBytes(2).toString('hex');
    const generatedUsername = `darkpalzy-${randomUserBytes}`;
    const generatedKey = crypto.randomBytes(4).toString('hex');
    const expiredTimestamp = Date.now() + durationMs;

    usersData.push({ username: generatedUsername, key: generatedKey, expired: expiredTimestamp });
    fs.writeFileSync(pathUserJson, JSON.stringify(usersData, null, 2), "utf-8");

    res.cookie('has_generated_key', 'true', { maxAge: durationMs, httpOnly: true });
    const expiredDate = new Date(expiredTimestamp).toLocaleString("id-ID", { timeZone: "Asia/Jakarta" });
    
    resultTemplate = resultTemplate
      .replace('{{STATUS_CLASS}}', 'success')
      .replace('{{TITLE}}', '✅ Key Generated Successfully')
      .replace('{{MESSAGE}}', `<b>Username:</b> <code>${generatedUsername}</code><br><b>Key:</b> <code>${generatedKey}</code><br><b>Durasi:</b> 3 Hari (Locked)<br><b>Expired On:</b> ${expiredDate} WIB`)
      .replace('{{ACTION_BUTTON}}', '<a class="btn-redirect" href="/login"><i class="fa-solid fa-right-to-bracket"></i> PROCEED TO LOGIN</a>');
    ...
    resultTemplate = resultTemplate
      .replace('{{STATUS_CLASS}}', 'danger')
      .replace('{{TITLE}}', 'Server Error')
      .replace('{{MESSAGE}}', 'Gagal memproses database.')
      .replace('{{ACTION_BUTTON}}', '<a class="btn-redirect" href="/getkey"><i class="fas fa-arrow-left"></i> Kembali</a>');
    return res.send(resultTemplate);
  }
});
...
<body>
  ...
  <div class="sidebar" id="sidebar">
    ...
    <ul class="nav-menu">
      ...
      <li class="nav-item"><a href="/profile" class="nav-link"><i class="fas fa-user"></i>Profile</a></li>
      <li class="nav-item"><a href="https://t.me/darkpalzy" class="nav-link"><i class="fab fa-telegram"></i>Telegram</a></li>
      <li class="nav-item"><a href="https://wa.me/6283820463478" class="nav-link"><i class="fab fa-whatsapp"></i>WhatsApp</a></li>
      <li class="nav-item"><a href="/chat-ai" class="nav-link"><i class="fas fa-robot"></i>Chat AI</a></li>
  ...
  <div class="main-content">
      ...
      <div class="bug-selector-container">
        ...
        <div class="selected-bug-display" id="selectedBugDisplay">
          <div class="terminal-line">
            <span class="terminal-prompt">user@DarkPalzy:~$</span>
    ...
    <div class="info-section">
      ...
      <div class="info-content">
        ...
        <p>DarkPalzy menggunakan protokol keamanan tingkat tinggi untuk melindungi identitas dan data pengguna:</p>
